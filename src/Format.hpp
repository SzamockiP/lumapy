...
constexpr FormatInfo format_info(Format f)
{
	switch (f)
	{
		case Format::RGBA8:      return { VK_FORMAT_R8G8B8A8_UNORM,       4,  4, "uint8",   false };
		case Format::RGBA8_SRGB: return { VK_FORMAT_R8G8B8A8_SRGB,        4,  4, "uint8",   false };
		case Format::BGRA8:      return { VK_FORMAT_B8G8R8A8_UNORM,       4,  4, "uint8",   false };
		case Format::R8:         return { VK_FORMAT_R8_UNORM,             1,  1, "uint8",   false };
		case Format::RG8:        return { VK_FORMAT_R8G8_UNORM,           2,  2, "uint8",   false };
		case Format::R16F:       return { VK_FORMAT_R16_SFLOAT,           2,  1, "float16", false };
		case Format::RGBA16F:    return { VK_FORMAT_R16G16B16A16_SFLOAT,  8,  4, "float16", false };
		case Format::R32F:       return { VK_FORMAT_R32_SFLOAT,           4,  1, "float32", false };
		case Format::RGBA32F:    return { VK_FORMAT_R32G32B32A32_SFLOAT, 16,  4, "float32", false };
		case Format::D32F:       return { VK_FORMAT_D32_SFLOAT,           4,  1, "float32", true };
	}
