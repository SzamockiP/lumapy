...
{
public:
	explicit UploadManager(Context& context) : context_(context)
	{
	...
	void wait_all() override
	{
		std::uint64_t wait_serial = 0;
		{
			...
			// failed) …
			cv_.wait(lock, [&] {
				return failed_count_ + submitted_serials_.size() == batch_started_;
			});
...
private:
	...
	std::uint64_t done_count_()  // call with mutex_ held
	{
		...
		const auto gpu_done = static_cast<std::uint64_t>(std::ranges::count_if(
			submitted_serials_, [&](std::uint64_t s) { return s <= completed; }));
	...
	void fail_(Job& job, const char* reason)
	{
		const std::string message = reason
			? std::format("Failed to load image: {} ({})", job.path, reason)
